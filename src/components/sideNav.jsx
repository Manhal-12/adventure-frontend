...
function Sidebar() {
  ...
  const handleGetIncome = () => {
    axios.get("https://adventure-backend-4.onrender.com/getIncome/order").then((res) => {
      setTotalIncome(res.data[0].totalIncome);
    });
  };

  const handleTopCustomer = () => {
    axios.get("https://adventure-backend-4.onrender.com/getTopCustomer/order").then((res) => {
